...
export function UpdatePostModal({ post, open, setOpen }: UpdatePostModalProps) {
  ...
  }, [post.title, post.content, post.category, post.slug, form]);
  ...
  const { mutate: updatePost, status: updatePostStatus } = useMutation({
    mutationFn: () => {
      return updatePostAction(post.id, form.getValues());
    },
    onSuccess: (data) => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <Form {...form}>
          <form>
            ...
            <DialogFooter>
              {updatePostStatus === "pending" && <Spinner className="mr-2" />}
              <Button
                type="button"
                onClick={() => handleSubmit("published")}
                disabled={updatePostStatus === "pending"}
              >
                발행
              </Button>
              {post.state === "draft" && (
